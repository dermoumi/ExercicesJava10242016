 ...
 */
package matriceslignes;

/**
 * Calcule le produit de deux matrices ligne par ligne
 *
 * @author Dermoumi S. (sdermoumi@gmail.com)
 */
public class CalculLigne implements Runnable {
    ...
    public void run() {
       for (int j = 0; j < MAX_COL; ++j) {
           matResultat[ligne][j] = 0.0;
           for (int k = 0; k < MAX_DIM_COM; ++k) {
               matResultat[ligne][j] += mat1[ligne][k] * mat2[k][j];
           }
       }
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < MAX_LIG; ++i) {
            Thread th = new Thread(thGrp, new CalculLigne(i));
            th.start();
        }
